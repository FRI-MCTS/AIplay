...
*/
Player_AI_UCT_RAVE::Player_AI_UCT_RAVE(Game_Engine* game, int player_number) : Player_Engine(game, player_number)
{
//--
	//player definition
	player_name = "UCT_RAVE";
...
//destructor
Player_AI_UCT_RAVE::~Player_AI_UCT_RAVE(void)
{
	Clear_Memory();
}

//allocate memory and initialize variables
void Player_AI_UCT_RAVE::Initialize()
{
	//set init flag
	is_initialized = true;
...
void Player_AI_UCT_RAVE::Init_Settings()
{
	//init internal variables
	all_actions_num = game->maximum_allowed_moves;
	UCTtree_maxMemorySize = (int)(TOMPLAYER_AI_UCT_RAVE_TREE_SIZE_LIMIT_GB * (1<<30));
	UCTtree_num_nodes = 0;
	UCTtree_memorySize = 0;

	//init optimization settings
	internal_game_force_copy = TOMPLAYER_AI_UCT_RAVE_OPTIMIZATION_INTERNAL_FORCE_COPY;

	//init learning parameters
	UCT_param_C = TOMPLAYER_AI_UCT_RAVE_PARAM_C;
	UCT_param_IterNum = TOMPLAYER_AI_UCT_RAVE_PARAM_NUMBER_ITERATIONS;
	UCT_param_defaultPolicy_maxPlys = TOMPLAYER_AI_UCT_RAVE_PARAM_DEFAULT_MAX_PLAYS;
	UCTtree_preserve = TOMPLAYER_AI_UCT_RAVE_PARAM_PRESERVE_TREE;
	
	//RAVE
	RAVE_param_V = TOMPLAYER_AI_UCT_RAVE_PARAM_V;
}

void Player_AI_UCT_RAVE::Allocate_Memory()
{
//--
	//allocate resources
	...
	//array for All-Moves-As_First (RAVE) heuristic
	RAVE_flagList = new int[all_actions_num];

}

void Player_AI_UCT_RAVE::Clear_Memory()
{
//--
	//clean up memory
	if(is_initialized){
		...
		delete(RAVE_flagList);
	}
}

void Player_AI_UCT_RAVE::New_Game2()
{
...
void Player_AI_UCT_RAVE::New_Game()
{
...
int Player_AI_UCT_RAVE::Get_Move()
{
//--
	int moveNumber;
	...
	//Execute UCT RAVE algortihm and select best action
	moveNumber = UCT_RAVE();

	//DEBUG
#if(TOMPLAYER_AI_UCT_RAVE_DEBUG_ROOT_CHILDREN_VALUES)
	Output_UCT_Tree_Node_Children(UCTroot);		//show values of roots actions (children)
#endif
#if(TOMPLAYER_AI_UCT_RAVE_VISUALIZE_UCT_GETC_AFTER_MOVE)
	cin.get();
#endif
...
void Player_AI_UCT_RAVE::UCT_Update_Internal_Game(int number_actions)
{
...
#if(TOMPLAYER_AI_UCT_RAVE_DEBUG_HISTORY_COPY_CHECK)
...
int Player_AI_UCT_RAVE::UCT_RAVE()
{
//--
	...
	for(int i = 0; i < UCT_param_IterNum; i++){
		...
		//DEBUG
#if(TOMPLAYER_AI_UCT_RAVE_VISUALIZE_LEVEL_UCT)
		printf("\nSim num %2d     score",i);
...
#endif
#if(TOMPLAYER_AI_UCT_RAVE_VISUALIZE_UCT_ACTIONS_PLAYOUT)
		printf("    \t playout:");
...
#if(TOMPLAYER_AI_UCT_RAVE_DEBUG_TREE_EXPANSION_SIM)
		Output_UCT_Tree();
#endif
...
//--
	...
	while(terminalNode == 0){
		...
		if(currentNode->number_explored_children >= simulatedGame->current_number_moves[simulatedGame->current_player]){
			...
			currentNode = UCT_RAVE_Tree_Policy_Best_Child(currentNode, UCT_param_C);
			...
			//DEBUG
#if(TOMPLAYER_AI_UCT_RAVE_DEBUG_TRACE_BEST_UCT)
...
*/
Player_AI_UCT_RAVE::UCTnode* Player_AI_UCT_RAVE::UCT_RAVE_Tree_Policy_Best_Child(UCTnode* parent, double param_C){
//--
	double bestValue;
	...
	for(int i = 0; i < parent->number_allowed_actions; i++){
		...
		if(parent->children[i] != NULL){
			...
			//---- UCB + RAVE equation ----
			//variant #1 = a*RAVE + (1-a)*UCT
			//double UCBval = (child->rewards / child->visits) + 2.0 * param_C * sqrt(2*log(parent->visits) / child->visits);
			//double alpha;
			//if(RAVE_param_V >= 1)
			//	alpha = max(0.0, (RAVE_param_V - child->visits) / RAVE_param_V);
			//else
			//	alpha = 0.0;
			//double RAVEval;
			////if(child->RAVE_visits != 0.0)
			//	RAVEval = (child->RAVE_rewards + 0.5) / (child->RAVE_visits + 1);	//TODO: may be speeded up by including in previous if-sentence
			////else
			////	RAVEval = 0.0;
			//child->value = alpha*RAVEval + (1-alpha)*UCBval;

			//variant #2 = a*RAVE + (1-a)*Q/N + c*UCT_EXP
			double alpha;
			if(RAVE_param_V >= 1)
				alpha = max(0.0, (RAVE_param_V - child->visits) / RAVE_param_V);
			else
				alpha = 0.0;
			double MCval = (child->rewards / child->visits);
			...
			//else
			//	RAVEval = 0.0;
			child->value = alpha*RAVEval + (1-alpha)*MCval + 2.0 * param_C * sqrt(2*log(parent->visits) / child->visits);
...
Expand the tree
*/
Player_AI_UCT_RAVE::UCTnode* Player_AI_UCT_RAVE::UCT_Expand(UCTnode* parent){
...
#if(!TOMPLAYER_AI_UCT_RAVE_DEBUG_DISABLE_RANDOM)
	randAction = (int)((rand()/(double)(RAND_MAX + 1.0))*(parent->number_allowed_actions - parent->number_explored_children)); //TODO overflow
#else
	randAction = 0;
#endif
...
*/
double* Player_AI_UCT_RAVE::UCT_RAVE_Default_Policy(int simulation_number){
//--
	...
	while((gameStatus == 0)&&( (simulatedGame->current_plys - internalGame->current_plys) < UCT_param_defaultPolicy_maxPlys)){
		...
		//DEBUG
#if((TOMPLAYER_AI_UCT_RAVE_VISUALIZE_LEVEL_UCT)&&TOMPLAYER_AI_UCT_RAVE_VISUALIZE_UCT_ACTIONS_TREE)
		//sprintf(visualizeActionsTree, "  > %2d", currentNode->action_index);
		visualizeActionsPlayout << "  > " << lastMove;
		//printf("  > %2d", currentNode->action_index);
#endif
#if(TOMPLAYER_AI_UCT_RAVE_DEBUG_SIMULATED_GAMES_OUT)
		simulatedGame->Output_Board_State();
		//simulatedGame->output_state_to_STD();
		//simulatedGame->output_chains_to_STD();
		//printf("Moves: %d %d\n",simulatedGame->num_moves[0], simulatedGame->num_moves[1]);
		...
		//RAVE heuristic: remember moves
		RAVE_flagList[lastMove] = simulation_number+1;

//todo: pseudocode some-first-amaf, pseudocode: 
//#if(SOME_FIRST_RAVE_ENABLED)
//		if(num_simulated_plys < some_first_threshold)
//			RAVE_flagList[lastMove] = simulation_number;
//#endif
...
*/
void Player_AI_UCT_RAVE::UCT_RAVE_Backup(UCTnode* leaf, double* rewards, int simulation_number){
//--
	...
	for(int i = 0; i < UCTtree_treePolicy_lastDepth+1; i++){
		...
		//unflag RAVE for nodes selected by tree-policy
		//RAVE_flagList[currentNode->action_index] = 0;

		//move to parent
		currentNode = currentNode->parent;						

		//RAVE update of children (alternative first moves)
		#if(!TOMPLAYER_AI_UCT_RAVE_DISABLE_AMAF)
		if(currentNode != NULL){ //TODO: could be avoided if the for i loop would go to 1 element less
			#if(TOMPLAYER_AI_UCT_RAVE_IGNORE_UNTIL_EXPANDED)
			if(currentNode->number_explored_children == currentNode->number_allowed_actions){
				...
				for(int i = 0; i < currentNode->number_allowed_actions; i++){				
					if(currentNode->children[i] != NULL){	//check if children was already allocated (for safety)
						if(RAVE_flagList[currentNode->children[i]->action_index] == (simulation_number+1) ){
							currentNode->children[i]->RAVE_visits += 1.0;
							currentNode->children[i]->RAVE_rewards += rewards[belonging_player];
						}
					}
				}
			#if(TOMPLAYER_AI_UCT_RAVE_IGNORE_UNTIL_EXPANDED)
...
*/
Player_AI_UCT_RAVE::UCTnode* Player_AI_UCT_RAVE::UCT_Tree_Node_Init(int action, UCTnode* parent)
{
	UCTnode* node;
...
*/
void Player_AI_UCT_RAVE::UCT_Tree_Node_Reset(UCTnode* node)
{
//--
	node->visits = 0.0;
	node->rewards = 0.0;
	...
	//RAVE
	node->RAVE_visits = 0.0;
	node->RAVE_rewards = 0.0;	
}

/**
UCT Tree Node - allocate children (actions) list
*/
void Player_AI_UCT_RAVE::UCT_Tree_Node_AllocateList(UCTnode* node, int list_length)
{
...
void Player_AI_UCT_RAVE::UCT_Tree_Change_Root(int number_actions)
{
	...
	if(number_actions > 0){
		...
		if(!UCTtree_preserve){
			...
			//new root not found, selected actions were not present in tree
				...
				//DEBUG
#if(TOMPLAYER_AI_UCT_RAVE_DEBUG_MEMORY_SIZE_COUNT)
...
*/
void Player_AI_UCT_RAVE::UCT_Tree_Trim(UCTnode* branchRoot, int remaining_depth)
{
...
WARNING: does not delete first call branchRoot node
*/
void Player_AI_UCT_RAVE::UCT_Tree_Delete(UCTnode* branchRoot)
{
...
*/
void Player_AI_UCT_RAVE::UCT_Tree_Reset()
{
...
//TODO
void Player_AI_UCT_RAVE::UCT_Tree_Preserve()
{
}
//OLD CODE
...
*/
void Player_AI_UCT_RAVE::Output_UCT_Tree()
{
...
void Player_AI_UCT_RAVE::Output_UCT_Tree_Node_Children(UCTnode* parent)
{
//--
	...
	for(int i = 0; i < UCTroot->number_allowed_actions; i++){
		if(UCTroot->children[i] != NULL){
			printf("   %3d:  %5.1f  %5.1f  %5.1f  %5.1f  \t%3.3f\n",
				...
				UCTroot->children[i]->RAVE_visits, 
				UCTroot->children[i]->RAVE_rewards, 
...
*/
void Player_AI_UCT_RAVE::Output_UCT_Tree_Branch(UCTnode* branchRoot)
{
	UCTnode* tmpNode;
...
void Player_AI_UCT_RAVE::Debug_UCT_Tree_MemoryState()
{
...
int Player_AI_UCT_RAVE::Debug_UCT_Tree_Size(UCTnode*)
{
	return 0;
}

...
Player_AI_UCT_TomTest::UCTnode* Player_AI_UCT_TomTest::UCT_Tree_Policy_Best_Child(UCTnode* parent, double exploration_weight){
	...
	if(((simulatedGame->current_plys - internalGame->current_plys) % 2 == 1) && ((rand()/(double)(RAND_MAX + 1.0)) < TOMPLAYER_AI_UCT_TOMTEST_RANDOM_TREE_OPPONENT_THRESHOLD)){ //TODO overflow
...
//use this in combination with LRP_improved_v1() and LRP_test_wrapper()
				...
				set_QVALUE_param_scoreWeight =	netInputs[2] + QVALUE_param_scoreWeight;
...
// ---- END - DEBUG PRINT ----//
	...
	if(multiple_best > 1){
		randAction = (int)floorf( (rand()/(double)(RAND_MAX + 1.0)) * multiple_best ); //TODO overflow
		for(int i = 0; i < parent->number_allowed_actions; i++){
...
// -- Q-value parameter search and approximation -- //
	...
	set_QVALUE_param_alpha = Func_App_UCT_Params->parameter_weights[3] + QVALUE_param_alpha;
	...
	set_QVALUE_param_lambda_playOut = Func_App_UCT_Params->parameter_weights[5] + QVALUE_param_lambda_playOut;
...
// -- end - Q-value parameter search and approximation -- //
	...
	//double TD_leaf_state_value = 0.0;

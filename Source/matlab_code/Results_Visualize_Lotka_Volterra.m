...
plot_error_limit_value = 20; %set error plot Y axis limits

plot_disable_prey = ~plot_state_axis_limit;		%do not plot prey on first plot

%CStr = dataread('file', 'file_conf', '%s', 'delimiter', '\n')
c_config = fileread([inputPath file_conf]);

%init script
scriptName = 'Lotka Volterra';
...
error = results(:,6);
gamma = results(:,7);
changeRate = results(:,8);

%plot prey and predator
sp1 = axes('position',[0.2 0.55 0.3 0.35]);
if(~plot_disable_prey)
	plot(prey,'r','LineWidth',plot_lines_width);
	hold on
end
...
xlabel('Time steps');
if(plot_disable_prey)
	legend('predator','target','Location','NorthOutside','Orientation','horizontal');
else
	legend('prey','predator','target','Location','NorthOutside','Orientation','horizontal');
end
if(plot_state_axis_limit)
    if(min(min(prey),min(predator)) >= 0)
        if(max(abs(target)) > 0)
            ylim([ max(0,min(target)-max(target)) (max(target)*2)])
        else
            ylim([ 0 plot_error_limit_value])
        end
    else
        if(max(abs(target)) > 0)
            ylim([ (min(target)-max(target)) max(target)*2 ])
        else
            ylim([ -plot_error_limit_value  plot_error_limit_value])
        end
    end
end
...
hold on
plot(changeRate,'g','LineWidth',plot_lines_width);
hold off
legend('gamma','change factor','Location','NorthOutside','Orientation','horizontal' );

%plot actions (or state without axis-limit)
sp4 = axes('position',[0.61 0.08 0.3 0.35]);
if(~plot_state_axis_limit)
    plot(actions,'x','LineWidth',plot_lines_width);
    legend('actions','Location','NorthOutside','Orientation','horizontal' );
else 
    plot(prey,'r','LineWidth',plot_lines_width);
    hold on
    plot(predator,'b','LineWidth',plot_lines_width);
